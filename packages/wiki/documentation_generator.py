...
class DocumentationGenerator:
    ...
    async def generate_module_documentation(self, components: Dict[str, Any], leaf_nodes: List[str]) -> str:
        ...
        if len(module_tree) > 0:
            ...
            file_manager.save_json(final_module_tree, os.path.join(working_dir, _settings.WIKI_MODULE_TREE_FILENAME))

            # rename repo_name.md to overview.md
            repo_overview_path = os.path.join(working_dir, f"{repo_name}.md")
            if os.path.exists(repo_overview_path):
                os.rename(repo_overview_path, os.path.join(working_dir, _settings.WIKI_OVERVIEW_FILENAME))
    ...
    async def generate_parent_module_docs(self, module_path: List[str], 
        ...
        parent_docs_path = os.path.join(working_dir, f"{module_name if len(module_path) >= 1 else _settings.WIKI_OVERVIEW_FILENAME.replace('.md', '')}.md")
